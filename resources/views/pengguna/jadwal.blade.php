...
<form action="" method="GET">
    <div class="row">
        <div class="col-3">
            ...
            <input type="text" value="{{ old('nama_maskapai') }}" name="nama_maskapai" id="nama_maskapai" class="form-control" placeholder="Cari Nama Maskapai...">
        </div>
        ...
        @foreach ($maskapai as $data)
            <div class="col-6">
                <div class="card mb-3" style="max-width: 540px;">
                    <div class="row g-0">
                        <div class="col-md-4">
                            <img src="/storage/images/{{ $data->logo_maskapai }}" class="img-fluid rounded-start" alt="...">
                        </div>
                        <div class="col-md-8">
                            <div class="card-body">
                            <h5 class="card-title">{{ $data->nama_maskapai }}</h5>
                            <p class="card-text">{{ $data->rute->rute_asal}} Ke {{ $data->rute->rute_tujuan}} | {{ $data->rute->tanggal_pergi}} | {{ $data->jadwal->waktu_berangkat}} | Waktu Tiba: {{ $data->jadwal->waktu_tiba }}</p>
                            <div class="d-flex">
                                <p class="card-text d-inline"><p class="text-body-secondary">Rp.{{ number_format($data->jadwal->harga) }}</p>
                                <a href="/pengguna/jadwal/{{ $data->jadwal->id }}" class="d-inline ms-4">Pesan</a></p></p>
                            </div>
